...
- (textViewHeightDidChangedBlock)wzb_textViewHeightDidChanged {
    void(^textViewHeightDidChangedBlock)(CGFloat currentHeight) = objc_getAssociatedObject(self, WZBTextViewHeightDidChangedBlockKey);
    return textViewHeightDidChangedBlock;
}
...
- (void)wzb_autoHeightWithMaxHeight:(CGFloat)maxHeight {
    [self wzb_autoHeightWithMaxHeight:maxHeight textViewHeightDidChangedBlock:nil];
}
// 是否启用自动高度，默认为NO
static bool autoHeight = NO;
- (void)wzb_autoHeightWithMaxHeight:(CGFloat)maxHeight textViewHeightDidChangedBlock:(textViewHeightDidChangedBlock)textViewHeightDidChangedBlock {
    
    autoHeight = YES;
    [self wzb_placeholderView];             //监听键盘改变通知
    self.wzb_maxHeight = maxHeight;
    if (textViewHeightDidChangedBlock) {
        self.wzb_textViewHeightDidChanged = textViewHeightDidChangedBlock;
    }
}
    ...
    if (self.wzb_maxHeight >= self.bounds.size.height) {
        ...
        if (currentHeight != self.lastHeight) {
            ...
            if (currentTextViewHeight > self.wzb_minHeight) {
                ...
                self.frame = frame;
                // 调用block
                if (self.wzb_textViewHeightDidChanged) {
                    self.wzb_textViewHeightDidChanged(currentTextViewHeight);   //get方法
                }
                // 记录当前高度
                ...
                CGRect frame = self.frame;
                frame.size.height = self.wzb_minHeight;
                self.frame = frame;
                // 调用block
                if (self.wzb_textViewHeightDidChanged) {
                    self.wzb_textViewHeightDidChanged(self.wzb_minHeight);   //get方法
                }
                // 记录当前高度
                self.lastHeight = self.wzb_minHeight;
            }
        }
    }
...
- (void)wzb_addImage:(UIImage *)image size:(CGSize)size index:(NSInteger)index multiple:(CGFloat)multiple {
    if (image) {
        [self.wzb_imageArray addObject:image];
    }
